...
chain. So far it seems that L<Data::Query's|/Data::Query migration>
...
This code is well tested (yes it has a dedicated test file to just test its
output), however it is very weirdly written and hence hard to maintain. It
will also need to learn about dash-prefxed hash keys being names in order to
support L</Relationship arguments>.

=begin for NOT_PROPERLY_DESCRIBED_YET
